...
// RolesFromFiber returns a defensive copy of the authenticated roles.
func RolesFromFiber(c *fiber.Ctx) ([]string, bool) {
	principal, ok := PrincipalFromFiber(c)
	if !ok {
		return nil, false
	}
	return append([]string(nil), principal.Roles...), true
}

// PermissionsFromFiber returns a defensive copy of the authenticated permissions.
func PermissionsFromFiber(c *fiber.Ctx) ([]string, bool) {
	principal, ok := PrincipalFromFiber(c)
	if !ok {
		return nil, false
	}
	return append([]string(nil), principal.Permissions...), true
}
